...
One redeeming quality is that the brute-force method provides a very interesting visualisation of recursive backtracking. It may also help when a puzzle requires the use of strategies that haven't yet been implemented into the Solver- comparison with the solution found through backtracking can assist a user in searching for the missing strategy. The user can also play around with improving the rate of brute forcing the puzzle by finding candidates (squares with fewest candidates will be guessed first, reducing the amount of backtracking necessary).
